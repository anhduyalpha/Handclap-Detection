...
from ..config import settings, SensitivityPresets, save_persistent_settings
...
class SettingsUpdateRequest(BaseModel):
    preset: Optional[str] = None # "high_sensitivity" | "balanced" | "strict_anti_noise"
    energy_threshold: Optional[float] = None
    crest_factor_min: Optional[float] = None
...
def get_settings():
    return {
        ...
        "presets": SensitivityPresets.PRESETS,
...
def update_settings(req: SettingsUpdateRequest):
    # 1. Nếu chọn Preset nhanh -> Tự động nạp toàn bộ cấu hình tối ưu của Preset đó
    if req.preset and req.preset in SensitivityPresets.PRESETS:
        p = SensitivityPresets.PRESETS[req.preset]
        settings.dsp.energy_threshold = p["energy_threshold"]
        settings.dsp.crest_factor_min = p["crest_factor_min"]
        settings.dsp.hf_energy_ratio_min = p["hf_energy_ratio_min"]
        settings.ml.confidence_threshold = p["confidence_threshold"]
        live_engine.classifier.confidence_threshold = p["confidence_threshold"]
        settings.adaptive_noise.margin_factor = p["margin_factor"]
        settings.pattern.max_inter_clap_ms = p["max_inter_clap_ms"]

    # 2. Tinh chỉnh chi tiết từng thông số thủ công
    if req.energy_threshold is not None:
        settings.dsp.energy_threshold = req.energy_threshold
    if req.crest_factor_min is not None:
        settings.dsp.crest_factor_min = req.crest_factor_min
