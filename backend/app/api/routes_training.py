...
from ..config import settings, SensitivityPresets, USER_PROFILES_DIR, CHECKPOINTS_DIR
...
def create_profile(req: ProfileCreateRequest):
    clean_name = sanitize_identifier(req.name, "profile_name")
...
def list_samples(profile_name: str = "default", category: Optional[str] = None):
    ...
    clean_cat = sanitize_identifier(category, "category") if category else None
    samples = dataset_mgr.list_samples_detailed(clean_prof, clean_cat)
    return {
...
def upload_sample(req: SampleUploadRequest):
    ...
    try:
        ...
        try:
            ...
            auto_learner.notify_new_sample(clean_prof, clean_cat)
        except Exception as err:
            print(f"[RoutesTraining] AutoLearner note: {err}")

        claps, noises = dataset_mgr.load_dataset(clean_prof)
        return {
            "status": "success",
            "sample": sample_info,
...
def delete_sample(req: SampleDeleteRequest):
    ...
    clean_prof = sanitize_identifier(req.profile_name, "profile_name")
    clean_cat = sanitize_identifier(req.category, "category")
    clean_id = sanitize_identifier(req.sample_id, "sample_id")
    
    success = dataset_mgr.delete_sample(clean_prof, clean_cat, clean_id)
    if not success:
        raise HTTPException(status_code=404, detail="Mẫu âm thanh không tồn tại")
    return {"status": "success", "message": f"Đã xoá mẫu {clean_id}"}
...
def clear_samples(req: ClearCategoryRequest):
    ...
    count = dataset_mgr.clear_category(clean_prof, clean_cat)
...
def train_model(req: TrainRequest):
    ...
    try:
        ...
        settings.ml.active_profile = clean_prof
        live_engine.reload_model(clean_prof)

        return {
            "status": "success",
            "message": f"Đã huấn luyện và kích hoạt thành công mô hình '{clean_prof}'",
            "metrics": meta
        }
    except HTTPException:
        raise
...
def activate_profile(profile_name: str = Body(..., embed=True)):
    ...
    settings.ml.active_profile = clean_prof
    live_engine.reload_model(clean_prof)
    return {"status": "success", "active_profile": clean_prof}
...
def record_hardware_sample(req: HardwareRecordRequest):
    ...
    return {
        "status": "success",
        "message": f"Đã thu âm 2.0s từ Micro phần cứng Server và lưu vào danh mục '{clean_cat}'",
        "sample": sample_info,
        "category": clean_cat,
        "claps_count": len(claps),
        "noises_count": len(noises)
...
def record_continuous_session(req: ContinuousSessionRequest):
    ...
    cat_label = CATEGORIES.get(clean_cat, clean_cat)
    return {
        "status": "success",
...
def stream_sample_wav(profile_name: str, category: str, filename: str):
    """Phục vụ file âm thanh WAV để phát trực tiếp trên trình duyệt (Đã bảo vệ Path Traversal)"""
    wav_path = safe_path_resolve(USER_PROFILES_DIR, profile_name, category, filename)
...
def upload_checkpoint(
    ...
    _authorized: bool = Depends(verify_studio_token)
):
    """
    Nhận gói checkpoint mô hình AI vừa huấn luyện xong từ máy Windows và Hot-Reload trên Linux.
    Bảo mật: Yêu cầu X-Studio-Token và Whitelist tên tệp tin mô hình.
    """
    clean_prof = sanitize_identifier(req.profile_name, "profile_name")
    ckpt_dir = safe_path_resolve(CHECKPOINTS_DIR, clean_prof)
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    for fname, b64_content in req.files.items():
        # Kiểm tra Whitelist tên file mô hình
        if fname not in ALLOWED_CHECKPOINT_FILENAMES:
            raise HTTPException(
                status_code=400,
                detail=f"Tên tệp '{fname}' không nằm trong danh sách checkpoint được phép."
            )
        
        target_path = safe_path_resolve(ckpt_dir, fname)
    ...
    if live_engine.broadcast_callback:
        try:
            live_engine.broadcast_callback({
                ...
                "message": f"🚀 AI Model profile '{clean_prof}' vừa được tự động nâng cấp thành công!"
            })
        except Exception as e:
    ...
    return {
        "status": "success",
        "message": f"Đã nạp và kích hoạt mô hình AI mới cho profile '{clean_prof}'",
        "metrics": req.metrics
    }
